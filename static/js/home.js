...
y = x.slice(1,-1)
var data_from_json = JSON.parse(y);
if (data_from_json != undefined || data_from_json != null){
    json = []
    for (var i=1; i<=data_from_json.number_of_rows;i++){
        json.push({
            id: i,
            name: data_from_json[i].name,
            category: data_from_json[i].category,
            num: data_from_json[i].num,
            weight: data_from_json[i].weight,
            price: data_from_json[i].price,
            status: data_from_json[i].status,
            datetime: data_from_json[i].datetime,
            comment: data_from_json[i].comment,
        })
    }

    viewModel.items.removeAll();
    viewModel.items(json);
    document.getElementById("order").innerHTML = "Заказ №" + String(data_from_json.order_id)
    document.title = "Заказ №" + String(data_from_json.order_id)
}
